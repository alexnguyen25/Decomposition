"""E7b: MLP head on PANNs CNN14 embeddings (2048-d, AudioSet-pretrained)."""
...
def load_labels(partition):
    ...
    emb = np.load(SCRATCH / f"panns_{partition.replace('.csv', '')}.npz",
                  allow_pickle=True)
    x, keys = emb["embeddings"], list(emb["keys"])
    order = {str(k): i for i, k in enumerate(sample_key)}
    rows = [order[str(k)] for k in keys]
    y = (y_true[rows] >= 0.5).astype(np.float32)
    m = y_mask[rows].astype(np.float32)
    return x, y, m
...
def predict_probs(model, x):
    ...
    with torch.no_grad():
        for s in range(0, len(x), 1024):
            xb = torch.from_numpy(x[s:s + 1024]).to(DEVICE)
...
def main():
    ...
    torch.save(model.state_dict(), SCRATCH / "ckpt_E7_panns_head.pt")
    np.save(SCRATCH / "E7_test_probs.npy", test_probs)
    append_result({
        "name": "E7_panns_head", "minutes": round(minutes, 1),
    ...
    print(f"E7_panns_head: test macro-F1 @0.5 = {m_default['macro_f1']:.4f}, "
